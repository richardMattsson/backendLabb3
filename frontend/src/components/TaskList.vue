...
<template>
  <article>
    <b-card
      v-for="task in tasksList.slice(0, limit || tasksList.length)"
      :title="task.title"
      :key="task.taskId"
      :subtitle="task.price + ' kr'"
      class="mb-2"
    >
      <b-card-text>
        {{ task.description }}
      ...
      <b-card-text style="color: sienna">
        <span class="pi pi-map-marker"></span> {{ task.address }}</b-card-text
      >
